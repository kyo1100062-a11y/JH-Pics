...
# 1. 프런트엔드 빌드 테스트
echo -e "${YELLOW}📦 프런트엔드 빌드 테스트 중...${NC}"
cd ui

# 환경 변수 확인
if [ -z "$VITE_SUPABASE_URL" ] || [ -z "$VITE_SUPABASE_ANON_KEY" ]; then
  echo -e "${YELLOW}⚠️  환경 변수가 설정되지 않았습니다.${NC}"
  echo "로컬 빌드 테스트를 위해 .env.production 파일을 확인하세요."
  echo "Vercel 배포 시에는 Dashboard에서 환경 변수를 설정하세요."
else
...
if [ $? -ne 0 ]; then
  echo -e "${RED}❌ 프런트엔드 빌드 실패${NC}"
  exit 1
fi

echo -e "${GREEN}✅ 프런트엔드 빌드 완료${NC}"
...
echo ""
echo "다음 단계:"
echo "1. Vercel에서 프런트엔드 배포 확인"
echo "2. Supabase Dashboard에서 Functions 확인"
echo "3. 프로덕션 환경에서 전체 기능 테스트 진행"
echo ""
echo "테스트 시나리오: docs/production_test_scenarios.md 참고"
